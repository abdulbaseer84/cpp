...
int main() {
    ...
    int dp[n + 1];// dp[i] represents the number of ways to decode the substring s[0...i-1]
    ...
    for (int i = 2; i <= n; i++) {
        ...
        int two_digit = stoi(s.substr(i - 2, 2)); // Two digit decoding
        if (two_digit >= 10 && two_digit <= 26) {
            dp[i] += dp[i - 2];
        }
    }

    cout << dp[n] << endl; // output: 3
